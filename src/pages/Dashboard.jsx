...
const Dashboard = () => {
  ...
  const handleBlogSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        setEditorSuccess(editingBlogId ? 'Blog updated successfully!' : 'Blog created successfully!');
      ...
      setEditorError(err.response?.data?.message || 'Error occurred while saving blog.');
    } finally {
  ...
  const handleCreateCategory = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        setCatSuccess(`Category "${newCatName}" created successfully!`);
        setNewCatName('');
        setNewCatDesc('');
        setNewCatImage(null);
      ...
      setCatError(err.response?.data?.message || 'Error occurred while creating category.');
    }
  };

  const handleDeleteCategory = async (catId) => {
    if (window.confirm('Delete this category? This operation is non-reversible.')) {
      try {
  ...
  const handleDeleteUser = async (userId) => {
    if (window.confirm('Delete this user account? All their articles will also be recursively deleted.')) {
      try {
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-1.5 text-left md:col-span-2">
                        ...
                        <input
                          ...
                          className="w-full px-4 py-2.5 rounded-xl border border-slate-300 dark:border-dark-700 bg-slate-50 dark:bg-dark-950 text-slate-800 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500 text-sm"
                          required
                        />
                      </div>
                      ...
                      <div className="space-y-1.5 text-left md:col-span-2">
                        ...
                        <textarea
                          ...
                          className="w-full px-4 py-2 rounded-xl border border-slate-300 dark:border-dark-700 bg-slate-50 dark:bg-dark-950 text-slate-800 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500 text-sm h-20 resize-none"
                          required
                        />
                      </div>
                      ...
                      <div className="space-y-1.5 text-left">
                        ...
                        <select
                          ...
                          className="w-full px-4 py-2.5 rounded-xl border border-slate-300 dark:border-dark-700 bg-slate-50 dark:bg-dark-950 text-slate-800 dark:text-slate-500 focus:outline-none focus:ring-2 focus:ring-brand-500 text-sm"
                          required
                        >
