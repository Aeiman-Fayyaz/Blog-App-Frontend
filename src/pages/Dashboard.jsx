...
const Dashboard = () => {
  ...
  const draftBlogs = myBlogs.filter(blog => blog.status === 'draft' && !blog.isSharedByMe);
  ...
  // Delete blog
  const handleDeleteBlog = async (blogId, isShared = false) => {
    const message = isShared 
      ? 'Are you sure you want to remove this shared blog from your feed?' 
    ...
    if (confirmed) {
      try {
        const endpoint = isShared ? `/blogs/${blogId}/share` : `/blogs/${blogId}`;
        const method = isShared ? 'delete' : 'delete';
        ...
        if (data.success) {
          ...
          fetchDashboardData();
          toast.success(isShared ? 'Blog removed from your feed!' : 'Blog deleted successfully!', {
            position: 'top-center',
            autoClose: 3000,
          });
        }
        ...
        <aside className="w-full lg:w-64 flex-shrink-0 bg-surface dark:bg-dark-surface border border-border/60 dark:border-dark-border/80 p-5 rounded-2xl shadow-sm h-fit space-y-6">
          ...
          <div className="space-y-1">
            ...
            <button
              ...
              className={`flex w-full items-center px-3 py-2 text-sm font-semibold rounded-xl transition-all ${activeTab === 'drafts' ? 'bg-primary/10 dark:bg-primary/5 text-primary dark:text-primary' : 'text-text hover:bg-surface dark:text-dark-text dark:hover:bg-dark-surface/40'}`}
            >
              <FileEdit className="w-4 h-4 mr-2.5" />
              Drafts
            </button>
            <button
              onClick={() => {
            ...
            <>
                        ...
                        {myBlogs.slice(0, 3).map(b => (
                          <div key={b._id} className="flex items-center justify-between p-3.5 bg-slate-50 dark:bg-dark-950 border border-slate-100 dark:border-dark-850 rounded-xl">
                            <div className="truncate flex-1 pr-4">
                              ...
                              <div className="flex flex-wrap items-center gap-2 mt-2 text-[10px] text-slate-800">
                                <span>{new Date(b.createdAt).toLocaleDateString()}</span>
                                <Link to={`/blogs/${b.slug}`} className="text-brand-600 hover:text-brand-500 font-semibold">View</Link>
                              </div>
                            </div>
                ...
                <div className="space-y-6">
                  ...
                  {myBlogs.length === 0 ? (
                    <div className="py-20 text-center space-y-4 border border-dashed rounded-3xl border-slate-300 dark:border-dark-800">
                      <p className="text-slate-400 text-sm">You haven't authored or shared any articles yet.</p>
                    ...
                    <div className="space-y-3">
                      {myBlogs.map((blog) => (
                        <div key={blog._id} className={`p-4 rounded-xl border flex items-center justify-between ${blog.isSharedByMe ? 'bg-blue-50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-800' : 'bg-slate-50 dark:bg-dark-950 border-slate-200 dark:border-dark-800'}`}>
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              <h4 className="font-bold text-slate-900 dark:text-primary text-sm">{blog.title}</h4>
                              {blog.isSharedByMe && (
                                <span className="text-[10px] bg-blue-200 dark:bg-blue-900 text-blue-800 dark:text-blue-300 px-2 py-0.5 rounded font-semibold flex items-center gap-1">
                                  <span>🔁</span> Shared
                                </span>
                              )}
                            </div>
                            <div className="flex items-center gap-3 text-xs text-slate-600 dark:text-slate-400 mt-2">
                              <span>{blog.category?.name || 'Unassigned'}</span>
                              <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${blog.status === 'published' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-300' : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-950/40 dark:text-yellow-300'}`}>
                                {blog.status}
                              </span>
                            ...
                            {blog.isSharedByMe && blog.author && (
                              <p className="text-xs text-slate-500 dark:text-slate-500 mt-2">
                                Original author: <span className="font-semibold text-slate-700 dark:text-slate-400">{blog.author.name}</span>
                              </p>
                            )}
                          </div>
                          <div className="flex items-center gap-2 ml-4">
                            <Link
                              to={`/blogs/${blog.slug}`}
                              className="p-1.5 rounded hover:bg-slate-100 dark:hover:bg-dark-800 text-slate-400 hover:text-brand-500 transition-colors"
                              title="View Post"
                            >
                            ...
                            {!blog.isSharedByMe && (
                              <>
                                ...
                                <button
                                  onClick={() => handleDeleteBlog(blog._id, false)}
                                  className="p-1.5 rounded hover:bg-slate-100 dark:hover:bg-dark-800 text-slate-400 hover:text-red-500 transition-colors"
                                  title="Delete Post"
                                >
                            ...
                            {blog.isSharedByMe && (
                              <button
                                onClick={() => handleDeleteBlog(blog._id, true)}
                                className="p-1.5 rounded hover:bg-red-100 dark:hover:bg-red-900/20 text-slate-400 hover:text-red-500 transition-colors"
                                title="Remove share"
                              >
                                <Trash2 className="w-4 h-4" />
                              </button>
              ...
              {activeTab === 'drafts' && (
                <div className="space-y-6">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      <h2 className="text-2xl font-extrabold text-slate-900 dark:text-white">Drafts ({draftBlogs.length})</h2>
                      <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Review and continue editing your saved drafts.</p>
                    </div>
                    <button
                      onClick={() => changeTab('write')}
                      className="px-4 py-2 text-xs font-bold text-white bg-brand-600 hover:bg-brand-500 rounded-xl shadow-sm transition-all"
                  ...
                  {draftBlogs.length === 0 ? (
                    <div className="py-20 text-center space-y-4 border border-dashed rounded-3xl border-slate-300 dark:border-dark-800">
                      <p className="text-slate-400 text-sm">No draft posts saved yet. Save a post as draft from the editor to continue later.</p>
                      <button onClick={() => changeTab('write')} className="px-4 py-2 bg-brand-600 text-white rounded-full font-semibold text-xs">Create Draft</button>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {draftBlogs.map((blog) => (
                        <div key={blog._id} className="p-4 rounded-xl border bg-slate-50 dark:bg-dark-950 border-slate-200 dark:border-dark-800 flex items-center justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              <h4 className="font-bold text-slate-900 dark:text-primary text-sm">{blog.title}</h4>
                              <span className="text-[10px] bg-yellow-100 text-yellow-800 dark:bg-yellow-950/40 dark:text-yellow-300 px-2 py-0.5 rounded font-semibold">Draft</span>
                            </div>
                            <div className="flex items-center gap-3 text-xs text-slate-600 dark:text-slate-400 mt-2">
                              <span>{blog.category?.name || 'Unassigned'}</span>
                              <span className="flex items-center gap-1">
                      ...
                      {bookmarks.map((b) => (
                        <div key={b._id} className="p-4 bg-slate-50 dark:bg-dark-950 border border-slate-200/50 rounded-2xl flex items-center justify-between">
                          <div className="truncate flex-1 pr-3 text-left">
                            <h4 className="font-bold text-sm text-slate-800 dark:text-secondary truncate">
