...
const renderMarkdown = (markdown) => {
  ...
  // Basic blocks
  html = html.replace(/```([\s\S]*?)```/g, '<pre class="bg-surface dark:bg-dark-950 p-4 rounded-xl text-xs font-mono overflow-x-auto text-text dark:text-dark-text my-4 border border-border/50 dark:border-dark-800"><code>$1</code></pre>');
  html = html.replace(/`([^`\n]+)`/g, '<code class="bg-muted dark:bg-dark-950 px-1 py-0.5 rounded text-xs font-mono text-primary dark:text-dark-primary">$1</code>');
  ...
  html = html.replace(/^> (.*?)$/gm, '<blockquote class="border-l-4 border-secondary pl-4 py-1 italic bg-muted/20 dark:bg-dark-950/20 text-muted dark:text-dark-muted my-4">$1</blockquote>');
  html = html.replace(/!\[(.*?)\]\((.*?)\)/g, '<img src="$2" alt="$1" class="max-w-full rounded-2xl my-6 mx-auto shadow-md border border-border/50 dark:border-dark-800" />');
...
const BlogDetail = () => {
  ...
  const fetchBlogDetails = async () => {
    try {
      ...
      if (data.success) {
        ...
        const relRes = await API.get(`/blogs?category=${data.blog.category._id}&limit=4`);
        if (relRes.data.success) {
          // Filter out current blog post
          setRelatedBlogs(relRes.data.blogs.filter(b => b._id !== data.blog._id).slice(0, 3));
        }
  ...
  const handleEditComment = async (e, commentId) => {
    ...
    try {
      const { data } = await API.put(`/comments/${commentId}`, { content: editInput.trim() });
      if (data.success) {
        setComments(prev => prev.map(c => c._id === commentId ? { ...c, content: data.comment.content } : c));
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10 space-y-10">
      ...
      <header className="space-y-4">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4 border-y border-border dark:border-dark-border/80 py-4 mt-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <Link to={`/author/${blog.author?._id}`} className="font-bold text-text dark:text-dark-text hover:text-secondary">
                {blog.author?.name}
              </Link>
              <div className="flex items-center text-xs text-muted space-x-3 mt-0.5">
                <span className="flex items-center"><Calendar className="w-3.5 h-3.5 mr-1" /> {new Date(blog.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="relative flex items-center space-x-2 bg-muted dark:bg-dark-800 p-1 rounded-full border border-border/50 dark:border-dark-border">
              <button onClick={shareOnTwitter} className="p-1.5 rounded-full hover:bg-surface dark:hover:bg-dark-surface text-muted hover:text-secondary" title="Share on Twitter"><Twitter className="w-3.5 h-3.5" /></button>
              <button onClick={shareOnLinkedIn} className="p-1.5 rounded-full hover:bg-surface dark:hover:bg-dark-surface text-muted hover:text-secondary" title="Share on LinkedIn"><Linkedin className="w-3.5 h-3.5" /></button>
              <button onClick={copyShareLink} className="p-1.5 rounded-full hover:bg-surface dark:hover:bg-dark-surface text-muted hover:text-secondary" title="Copy link"><Link2 className="w-3.5 h-3.5" /></button>
            </div>
          </div>
        </div>

        {shareFeedback && (
          <div className="text-right text-xs font-bold text-primary dark:text-dark-primary animate-pulse">{shareFeedback}</div>
        )}
      </header>
      ...
      <section className="bg-surface dark:bg-dark-surface border border-border dark:border-dark-border p-6 rounded-3xl flex flex-col sm:flex-row items-center sm:items-start gap-4">
        ...
        <div className="space-y-2 flex-grow text-center sm:text-left">
          ...
          <p className="text-sm text-muted dark:text-dark-muted">
            {blog.author?.bio || 'Author on BlogVerse. Check out their recent published writings.'}
          </p>
        </div>
      </section>
      ...
      <section className="space-y-6 pt-6">
        ...
        {/* Comment Entry Area */}
        {user ? (
          <form onSubmit={handleAddComment} className="flex gap-4 items-start bg-surface dark:bg-dark-surface border border-border dark:border-dark-border p-4 rounded-2xl shadow-sm">
          ...
          <div className="bg-surface dark:bg-dark-surface border border-dashed border-border dark:border-dark-border p-6 rounded-2xl text-center">
            <p className="text-muted text-sm">Please sign in to join the conversation.</p>
            <Link to="/login" className="inline-block mt-3 text-xs font-bold text-white bg-primary hover:bg-secondary px-4 py-2 rounded-full">
              Login to Comment
            </Link>
          </div>
        )}
        ...
        {commentsLoading ? (
          ...
          <p className="text-center text-sm text-muted py-6">No discussions yet. Be the first to start the thread!</p>
        ) : (
            ...
            {rootComments.map((comment) => {
              ...
              return (
                <div key={comment._id} className="space-y-4">
                  ...
                  <div className="bg-surface dark:bg-dark-surface border border-border dark:border-dark-850 p-4 rounded-2xl shadow-sm space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2.5">
                        ...
                        <div>
                          <span className="font-bold text-sm text-text dark:text-dark-text">{comment.user?.name}</span>
                    ...
                    {hasEdited ? (
                      <form onSubmit={(e) => handleEditComment(e, comment._id)} className="space-y-2">
                        <textarea
                          value={editInput}
                        ...
                        <div className="flex justify-end space-x-2">
                          <button type="button" onClick={() => setActiveEditBox(null)} className="px-2 py-1 text-[10px] border rounded text-muted">Cancel</button>
                          <button type="submit" className="px-2 py-1 text-[10px] bg-primary text-white rounded font-semibold">Save</button>
                        </div>
                      </form>
                    ) : (
                  ...
                  {/* Reply Input block */}
                  {hasReplied && (
                    <form onSubmit={(e) => handleAddComment(e, comment._id)} className="ml-10 md:ml-12 flex gap-3 items-start bg-surface dark:bg-dark-surface border border-border dark:border-dark-border p-3 rounded-xl">
                      ...
                      <div className="flex-1 space-y-2">
                        ...
                        />
                        <div className="flex justify-end space-x-2">
                          <button type="button" onClick={() => setActiveReplyBox(null)} className="px-3 py-1 text-[10px] font-bold text-muted">Cancel</button>
                          <button type="submit" className="px-3 py-1 bg-primary hover:bg-secondary text-white rounded font-bold text-[10px]">Reply</button>
                        </div>
                      </div>
                    </form>
                  )}
                  ...
                  {replies.length > 0 && (
                    <div className="ml-10 md:ml-12 border-l border-border dark:border-dark-border pl-4 space-y-4 pt-1">
                      {replies.map((reply) => {
                        ...
                        return (
                          <div key={reply._id} className="bg-muted/50 dark:bg-dark-surface/50 border border-border/50 dark:border-dark-850/60 p-3.5 rounded-xl space-y-2">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center space-x-2">
                                <img src={reply.user?.avatar} alt={reply.user?.name} className="w-7 h-7 rounded-full object-cover" />
                                <div>
                                  <span className="font-bold text-xs text-text dark:text-dark-text">{reply.user?.name}</span>
                            ...
                            {/* Reply Text Body */}
                            {isRepEdit ? (
                              <form onSubmit={(e) => handleEditComment(e, reply._id)} className="space-y-2">
                                <textarea
                                  value={editInput}
                                  onChange={(e) => setEditInput(e.target.value)}
                                ...
                                />
                                <div className="flex justify-end space-x-2">
                                  <button type="button" onClick={() => setActiveEditBox(null)} className="px-2 py-1 text-[10px] text-muted">Cancel</button>
                                  <button type="submit" className="px-2 py-1 text-[10px] bg-primary text-white rounded">Save</button>
                                </div>
                              </form>
                            ) : (
                            ...
                            <div className="flex items-center space-x-3 text-[11px] font-semibold text-muted pt-1">
                              ...
                              {user && (reply.user?._id === user._id || user.role === 'admin') && (
      ...
      {relatedBlogs.length > 0 && (
        <section className="pt-10 border-t border-border dark:border-dark-border">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {relatedBlogs.map((b) => (
              <div key={b._id} className="bg-surface dark:bg-dark-surface border border-border/50 dark:border-dark-850 p-4 rounded-2xl shadow-sm space-y-3">
                <Link to={`/blogs/${b.slug}`} className="block aspect-[16/10] overflow-hidden bg-muted rounded-lg">
                  <img src={b.image} alt={b.title} className="w-full h-full object-cover hover:scale-105 transition-all duration-300" />
                </Link>
                <h4 className="font-bold text-sm text-text dark:text-dark-text line-clamp-2">
                  <Link to={`/blogs/${b.slug}`} className="hover:text-secondary transition-colors">{b.title}</Link>
                </h4>
                <div className="flex items-center justify-between text-[10px] text-muted">
                  <span>By {b.author?.name}</span>
                  <span className="flex items-center"><Clock className="w-3 h-3 mr-0.5" /> {b.readTime} min</span>
                </div>
              </div>
            ))}
          </div>
